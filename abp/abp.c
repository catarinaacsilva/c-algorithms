...
Nome : Catarina Alexandra Correia Silva               NMEC : 76399
...
/*********************************** Aula 9 ***********************************/
void deleteNext(PtABPNode *proot, int *Elem){
  if((*proot)->PtLeft == NULL){
    *Elem = (*proot)->Elem;
    PtABPNode *pAux = proot;
    *proot = (*pAux)->PtRight;
    ABPNodeDestroy(pAux);
    return;	
  }

  deleteNext(&(*proot)->PtLeft, Elem);
}

void deleteNode(PtABPNode *proot){
  PtABPNode *p = proot;
  ...
  deleteNext(&(*proot)->PtRight, &(*proot)->Elem);
}
...
{ /* função repetitiva - insira o seu codigo */  
  ...
  PtABPNode *pnode = NULL;
  for(pnode = proot; (*pnode) -> PtRight != NULL; pnode = &(*pnode) -> PtRight);
...
{ /* função repetitiva - insira o seu codigo */
  ...
  while(node != NULL) {
    ...
    if(elem == pvalue) {
      ...
      if(elem < pvalue && (rv == NULL || rv->Elem < elem)) {
        rv = node;
      }
...
{ /* função recursiva - insira o seu codigo */
  ...
  if(elem > pvalue) {
    PtABPNode tmp = ABPCeilValue(proot->PtLeft, pvalue);
    if(tmp != NULL && tmp->Elem < elem) {
      rv = tmp;
    } else {
      rv = proot;
    }
    ...
    rv = ABPCeilValue(proot->PtRight, pvalue);
    if(rv == NULL) {
      Error = NO_ELEM;
    }
  }
...
{ /* função recursiva - insira o seu codigo */
  ...
  if(proot->Elem < pvalue) {
    rv = 1 + ABPRank(proot->PtLeft, pvalue) + ABPRank(proot->PtRight, pvalue);
  } else {
    rv = ABPRank(proot->PtLeft, pvalue);
  }
...
{ /* função recursiva - insira o seu codigo */
  ...
  if(elem >= plow && elem <= phigh) {
    QueueEnqueue(pqueue, &elem);  
  }
...
{ /* função repetitiva - insira o seu codigo */ 
  ...
  int size = ABPSize(proot);
  PtStack pstack = StackCreate(size);
