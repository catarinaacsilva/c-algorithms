/*
Nome: Catarina Alexandra Correia Silva
NMEC: 76399
*/
...
{
  unsigned int Number;	/* número do vértice ou da aresta - number of the vertes or edge */
...
{ 
  ...
  if(pdig == NULL)
    return NO_DIGRAPH;
  else if(pdig->NVertexes == 0)
    return DIGRAPH_EMPTY;
  else if((vert = OutPosition(pdig->Head, pv)) == NULL) 
    return NO_VERTEX;

  PtVertex vertex = (PtVertex) (vert->PtElem);

  if(vertex->InDeg == 0 && vertex->OutDeg == 0)
    return DISC;
  else if(vertex->InDeg == 0 && vertex->OutDeg != 0)
    return 	SOURCE;
  else if(vertex->InDeg != 0 && vertex->OutDeg == 0)
    return SINK;
  else	
...
{ 
  ...
  VertexNumber(pdig, &nVerts);
  *pcent= (double)vertex->OutDeg/(nVerts - 1);
  return OK;
}

/* ir a todos os vertices e ver o primeiro que vai para mais vertices no digrafo */
...
{ 
  ...
  if(pdig == NULL)
    ...
    return DIGRAPH_EMPTY;
  else if (pv == NULL || pmax == NULL) 
    return NULL_PTR;

  double mc = 0.0;
  int mv = 1, i = 0;
  VertexOutDegreeCentrality(pdig, 1, &mc);
  for(i = 2; i <= pdig->NVertexes; i++) {
    double cc = 0.0;
    VertexOutDegreeCentrality(pdig, i, &cc);
    if(cc > mc) {
      mc = cc;
      mv = i;
    }
...
{ 
  ...
  unsigned int n = 0;
  double sum = 0.0;
  PtBiNode binode = vert->PtAdj;
  for(; binode != NULL; binode=binode->PtNext) {
    PtBiNode sucessor = OutPosition(pdig->Head, binode->Number);
    PtVertex vertex = (PtVertex) sucessor->PtElem;
    sum += (double)vertex->OutDeg;
    n++;
  }
...
{ 
  ...
  for(i = 1; i <= pdig->NVertexes; i++) {
    int type = VertexType(pdig, i);
    if(type == DISC) {
      QueueEnqueue(*pqueue, &i);
    }
  }
...
{ 
  ...
  unsigned int i = 1;
  for(; i <= pdig->NVertexes; i++) {
    if(i != pv) {
      vert = OutPosition(pdig->Head, i);
      PtBiNode binode = vert->PtAdj;
      for(; binode != NULL; binode=binode->PtNext) {
        int predecessor = binode->Number;
        if(predecessor == pv) {
          QueueEnqueue(*pqueue, &i);
        }
      }
...
{ 
  ...
  unsigned int i = 0, map[pdig->NVertexes];
  for(i = 1; i <= pdig->NVertexes; i++) {
    vert = OutPosition(pdig->Head, i);
    PtBiNode binode = vert->PtAdj;
    memset(map, 0, sizeof(unsigned int)*pdig->NVertexes);
    for(; binode != NULL; binode=binode->PtNext) {
      int predecessor = binode->Number;
      map[predecessor-1] = 1;
    }
    ...
    unsigned int j = 0;
    for(; j < pdig->NVertexes; j++) {
      if(map[j] == 0 && i != j+1) {
        unsigned int vertex = j+1;
        QueueEnqueue(*pqueue, &i);
        QueueEnqueue(*pqueue, &vertvertexex);
      }
    }
  }
